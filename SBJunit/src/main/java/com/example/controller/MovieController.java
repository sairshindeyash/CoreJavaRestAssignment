...
public class MovieController {
	...
	/*
	 * @PostMapping(value = "/movie/add") public String addMovie(@RequestBody Movie
	 * movie) { service.addMovie(movie); return "Movie added"; }
	 */
	...
	public ResponseEntity<Response> removeToDoById(@PathVariable("id") Integer id) throws MovieNotFound {
		...
		return new ResponseEntity<Response>(new Response(HttpStatus.OK.value(), "Movie has been deleted"),
				HttpStatus.OK);
	}
	...
	/*
	 * @GetMapping(value="/movie/{id}") public Movie getMoviebyId(@PathVariable
	 * Integer id){ return service.getMovieById(id).orElseThrow(()->new
	 * MovieNotFound("Movie with "+id + "not found")); }
	 */
	/*
	 * @PutMapping("/movie/{id}") public String updateMovieById(@PathVariable
	 * Integer id,@RequestBody Movie movie){ Movie
	 * movie1=service.getMovieById(id).orElseThrow(()->new
	 * MovieNotFound("Movie with "+id + "not found"));
	 * movie1.setName(movie.getName()); movie1.setGenre(movie.getGenre());
	 * service.addMovie(movie1); return "Updated movie : "+id; }
	 */
